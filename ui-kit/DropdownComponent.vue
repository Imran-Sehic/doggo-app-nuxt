    ...
    <div>
        ...
        <div>
            <div class="selectedOption" @click="async () => {
                await closeModal(!expanded);
            }">
                {{ option }}
            </div>
            <div v-if="expanded" v-click-outside="async () => await closeModal(false)" class="options"
                :class="{ closing: closing }" tabindex="0">
...
const closeModal = async (expandedValue: boolean) => {
