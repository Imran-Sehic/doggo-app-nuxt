    ...
    <div class="ctaSection">
      <div class="dropdowns">
        <DropdownComponent
          ...
          v-model:option="breed"
        />
        <DropdownComponent
          :list="[
            'All',
            ...(Object.keys(breeds).length > 0 ? breeds[breed] : []),
          ]"
          :type="DropdownType.SUB_BREED"
          v-model:option="subBreed"
        />
      </div>
...
import { fetchDogsByBreedAndSubBreed } from "~/composables/queries";
import { DropdownType, type Breeds, type ListingText } from "~/types";
...
const breed = ref<string>(Object.keys(props.breeds)[0]);
const subBreed = ref<string>(
  props.breeds[breed.value].length > 0 ? props.breeds[breed.value][0] : "All"
);
...
const onSearch = async () => {
  ...
  emit("update:dogs", dogsResponse.value.message as string[]);
